...
class PublishArticleTool < ApplicationTool
  ...
  description "Publish a multilingual article into Smarts article pages. Pass meta (category/subcategory/title/summary) and content (locale -> markdown body) inline; falls back to docs/drafts/:slug on the server if not provided. Optionally schedule X posts via x_queue. Requires the MCP connection to carry an Authorization: Bearer <publish_token> header."

  input_schema(
    properties: {
      slug: { type: "string", description: "Two-character article slug, lowercase letters and digits only." },
      meta: {
        type: "object",
  ...
  class << self
    def payload(slug:, meta: nil, content: nil, published_at: nil, dry_run: false, thread: true, tweets: {})
