...
class ProtocolAdapters::GenericErc20AdapterTest < ActiveSupport::TestCase
  ...
  # ---------- panel_data: only the 4 core ERC-20 fns are multicalled ----------

  test "panel_data multicalls only the 4 core ERC-20 view functions" do
    # Admin / role probing moved to AdminRisk::Profiler so this panel stays
    # focused on token economics — the multicall must NOT grow for stablecoins
    # whose ABI happens to expose paused / owner / etc.
    abi = ERC20_ABI + [
    ...
    contract = Contract.new(chain: @chain, address: USDC_ADDRESS, abi: abi)
    adapter = ProtocolAdapters::GenericErc20Adapter.new(contract)

    stub = lambda do |chain:, calls:|
      assert_equal 4, calls.length,
        "multicall must stay at 4 — admin probing belongs to AdminRisk::Profiler now"
      [
        ChainReader::Multicall3Client::Result.new(success: true, values: [ "USD Coin" ]),
        ChainReader::Multicall3Client::Result.new(success: true, values: [ "USDC" ]),
        ChainReader::Multicall3Client::Result.new(success: true, values: [ 6 ]),
    ...
    stub_class_method(ChainReader::Multicall3Client, :call, stub) do
      stub_class_method(DefiLlamaClient, :fetch_prices, no_price) do
        data = adapter.panel_data
        assert_equal "USDC", data[:symbol]
        refute data.key?(:admin_status), "admin_status must not appear in panel_data"
        refute data.key?(:admin_roles),  "admin_roles must not appear in panel_data"
      end
    end
  end
